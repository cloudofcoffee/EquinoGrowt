<template>
  <div class="flex flex-col min-h-screen">
    ...
    <nav class="bg-teal-950 text-white shadow-md">
      ...
      >
        ...
        <router-link to="/" class="flex items-center space-x-2">
          <img
            src="/img/EquinoGrowt_logo.svg"
            alt="EquinoGrowt Logo"
            class="w-40 h-auto"
          />
        </router-link>
        ...
        <ul class="hidden md:flex items-center space-x-4 text-base font-medium">
          <li v-if="!isLoggedIn">
            <router-link
              to="/bienvenidos"
              class="relative inline-block hover:text-[#cfe8e4] transition"
              exact-active-class="text-[#cfe8e4] transition active-link"
              >Bienvenidos</router-link
            >
          </li>
          <li v-if="!isLoggedIn">
            <router-link
              to="/login"
              class="relative inline-block hover:text-[#cfe8e4] transition"
              active-class="text-[#cfe8e4] transition active-link"
              >Login</router-link
            >
          </li>
          <li v-if="!isLoggedIn">
          ...
          <li v-if="isLoggedIn">
            <router-link
              to="/perfil"
              class="bg-teal-600 hover:bg-teal-700 text-white px-4 py-2 rounded-full transition duration-200 flex items-center space-x-1"
            >
          ...
          <li v-if="isLoggedIn">
            <button
              @click="logout"
              class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-full transition duration-200 flex items-center space-x-1"
              title="Cerrar sesión"
            >
      ...
      <transition name="fade">
        <ul
          v-if="menuOpen"
          class="md:hidden bg-teal-900 px-4 pb-4 space-y-3 font-medium"
        >
          <li v-if="!isLoggedIn">
            <router-link
              to="/bienvenidos"
          ...
          <li v-if="isLoggedIn && userRole === 'doctor'">
            <router-link
              to="/notas"
              class="hover:text-[#cfe8e4] transition"
              active-class="text-[#cfe8e4] transition "
              >Notas</router-link
            >
          </li>
          <li v-if="isLoggedIn">
            <button class="w-full text-left py-2 px-2 bg-teal-600 hover:bg-teal-700 text-white rounded-md transition duration-200">
              <router-link
                to="/perfil"
              >
    ...
    <footer class="bg-[#0e0e0e] text-gray-200 pt-10 border-t">
      <div
        class="max-w-7xl mx-auto px-6 md:px-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8"
      >
        <!-- Logo y descripción -->
        <div>
          <div class="flex items-center space-x-3 mb-4">
            <img
              src="/img/EquinoGrowt_logo.svg"
              alt="EquinoGrowt Logo"
              class="rounded bg-white p-2 w-14 h-14"
            />
            <span class="text-2xl font-bold">&copy; EquinoGrowt</span>
          </div>
          <p class="text-sm">
            EquinoGrowt ofrece equino terapia para mejorar el bienestar físico y
            emocional a través del vínculo con los caballos.
          </p>
        </div>
        ...
        <div>
          ...
          <ul class="space-y-2 text-sm">
            <li>
              <router-link to="/bienvenidos" class="hover:underline"
                >Bienvenidos</router-link
              >
            </li>
            <li v-if="!isLoggedIn">
            ...
            <li v-if="isLoggedIn">
              <router-link to="/perfil" class="hover:underline"
                >Perfil</router-link
              >
            </li>
            <li v-if="isLoggedIn && userRole === 'doctor'">
              <router-link to="/notas" class="hover:underline"
                >Notas</router-link
              >
            </li>
          </ul>
        ...
        <div>
          ...
          <ul class="space-y-2 text-sm">
            <li>
              <i class="fas fa-envelope mr-2"></i> contacto@equinogrowt.com
            </li>
        ...
        <div>
          ...
          <div class="flex space-x-4 text-xl">
            ...
            ></a>
            <a
              href="https://www.instagram.com/equinogrowt/?utm_source=ig_web_button_share_sheet"
              target="_blank"
              class="hover:text-teal-500 transition"
      ...
      <!-- Derechos -->
      <div
        class="text-center text-sm text-gray-200 hover:text-gray-400 transition pb-6"
      >
        &copy; 2025 <span class="font-semibold">EquinoGrowt</span>. Todos los
        derechos reservados.
      </div>
    </footer>
  </div>
  ...
  created() {
    ...
    auth.onAuthStateChanged(async (user) => {
      if (user) {
        ...
        if (docSnap.exists()) {
          this.userRole = docSnap.data().tipo;
        }
      } else if (
